<?php
function draw_contact_form( $atts ){
    global $location;
    ...
    if( stristr( $source, 'PPC') && $_GET['source']=='ppc'){
        $ppc_data = '<input type="hidden" name="ppc_campaign" id="contact-ppc_campaign" class="ppc_campaign" value="'.$_GET['cmp'].'"/>
                     <input type="hidden" name="ppc_adgp" id="contact-ppc_adgp" class="ppc_adgp" value="'.$_GET['ag'].'"/>
                     <input type="hidden" name="ppc_kw" id="contact-ppc_kw" class="ppc_kw" value="'.$_GET['kw'].'"/>';
    }
    ...
    $output = '<div class="contact_form '.$class.'">
                    ...
                    <form action="'.$action.'" method="'.$method.'" id="signup-form">
                        ...
                        <p><label for="contact-name" accesskey="N"><span class="required">*</span> First Name:</label>
                        ...
                        <p><label for="contact-phone" accesskey="P"><span class="required">*</span> Phone Number:</label>
                       ...
                       '<input type="hidden" name="page" id="contact-tracpage" class="tracpage" value="'.get_permalink().'"/>
                        <input type="hidden" name="country" id="contact-country" class="country" value="'.get_country().'"/>
    ...
    if($modal_box == "1"){
        $output .= '<!-- modal content -->
                    <div id="osx-modal-content">
                            <div id="osx-modal-title">Thank you for Contacting Select Property.</div>
                            ...
                            <div id="osx-modal-data">
                                ...
                                <div id="thanks-new">
                                    <h2>Thank You for contacting Select Property</h2>
                                ...
                                <div id="thanksFooter">
                                    Tel: +44 (0)161 322 2222 | Email: <a href="mailto:info@selectproperty.com" title="Email Us">info@selectproperty.com</a>
                                    <img src="http://www.selectproperty.com/wp-content/themes/SelectProperty_new/images/thanks/logo-bottom.png" alt="Select Property">
                                </div>
                            </div>
                    </div>';
    }else{
        if( stristr( $source, 'PPC') === false ){
            $output .= '<!-- modal content -->
            ...
            $output .= '<!-- modal content -->
                   <div class="basic-modal-content">Targeted PPC thanks message. May need to look at header redirect to thanks page for goal tracking?</div>';
        }
    }
...
function initialInsert( $data ){
    ...
    $data = array( $data['contact-name'], $data['contact-surname'], $data['contact-email'], $data['contact-phone'], $data['comment'], $data['country'], $data['page'], $data['development'], $data['source'], $data['emailtemp'], $data['ppc_campaign'], $data['ppc_adgp'], $data['ppc_kw'] );
    
    $dbins = $wpdb->query( 
        $wpdb->prepare("
            INSERT INTO $table
            ( name,surname,email,tel,comment,country,page,development,source,emailtemp,ppc_campaign,ppc_adgp,ppc_kw  )
...
function sp_sendmail( $email ){
    ...
    $template = get_stylesheet_directory().'/lib/email_templates/select-contact-confirmation.php'; 
    switch ($email['Enquiry_Nature__pc']){
        ...
        default:
            $email_title = '';
            $email_info  = '';
            $template = get_stylesheet_directory().'/lib/email_templates/'.$emailtemp.'.php';
            break;
    }   
...
function sendleadmail( $email ){
    ...
    $message = Swift_Message::newInstance()
        ...
        ->setFrom(array('info@selectproperty.com' => 'Select Property Info'))
        ->setTo(array('leads@selectproperty.com', 'martin.vernon@selectproperty.com'))
        ->setBody($text)
...
function set_propBase_data ( $data ){
    ...
    switch ($data['development']) {
        ...
        default:
            ...
            $proenquiry['Preferred_City__c']                 = '';
            $proenquiry['Original_Preferred_City__c']        = '';
            $proenquiry['Preferred_Development__c']          = 'Contact Us';  
            $proenquiry['Original_Preferred_Development__c'] = 'Contact Us';
    }

    // Mailing Country
    $proenquiry['pb__Country__pc'] = $data['country'];
    ...
    // Visitior Tracking
    include_once('includes/class.gaparse.php');
    $aux = new GA_Parse($_COOKIE);

    $campaignsource  = $aux->campaign_source;
    $campaignname    = $aux->campaign_name;
    $campaignmedium  = $aux->campaign_medium;
    $campaigncontent = $aux->campaign_content;
    $campaignterm    = $aux->campaign_term;

    // Tracking Keyword
    $proenquiry['Tracking_Keyword__c']          = ( $data['ppc_kw'] ) ? substr($data['ppc_kw'], 0, 100) : substr($campaignterm, 0, 100);
    // Original Tracking Keyword
    $proenquiry['Original_Tracking_Keyword__c'] = ( $data['ppc_kw'] ) ? substr($data['ppc_kw'], 0, 100) : substr($campaignterm, 0, 100);
    
    $first_visit           = $aux->first_visit;	
    $previous_visit        = $aux->previous_visit;	
    $current_visit_started = $aux->current_visit_started;	
    $times_visited         = $aux->times_visited;

    $tracking =  " Date of first visit: ".$first_visit;
    $tracking .= " Date of previous visit: ".$previous_visit;
    $tracking .= " Date of current visit: ".$current_visit_started;
    $tracking .= " Times visited: ".$times_visited;
    
    $proenquiry['Web_Tracking_Note__c'] = $tracking;
    ...
    //Source Category
    if( $proenquiry['Lead_Source_SP__c'] == 'website' ){
        //Source Information
        switch( site_url() ){
        ...
        $proenquiry['Source_Information__c']          = ( $data['ppc_campaign'] ) ? $data['ppc_campaign'] : 'No PPC Campaign Name';
        $proenquiry['Original_Source_Information__c'] = ( $data['ppc_campaign'] ) ? $data['ppc_campaign'] : 'No PPC Campaign Name';
        $proenquiry['Source_Category__c']             = ( $data['ppc_adgp'] ) ? $data['ppc_adgp'] : 'No PPC Ad Group';
        $proenquiry['Original_Source_Category__c']    = ( $data['ppc_adgp'] ) ? $data['ppc_adgp'] : 'No PPC Ad Group';
    } elseif ( $proenquiry['Lead_Source_SP__c'] == 'Email' || $proenquiry['Lead_Source_SP__c'] == '3rd Party Lead Gen' ) {
        $proenquiry['Source_Information__c']          = ( $data['ppc_campaign'] ) ? $data['ppc_campaign'] : '';
        $proenquiry['Original_Source_Information__c'] = ( $data['ppc_campaign'] ) ? $data['ppc_campaign'] : '';
        $proenquiry['Source_Category__c']          = '';
        $proenquiry['Original_Source_Category__c'] = '';
    }
    //Lead Creation Date
    //Original Lead Creation Date
    ...
    wpb_map( array(
        ...
        "params" => array(
           ...
           array(
              ...
              "param_name" => "dev",
              "value" => array("First Street" => "first street",
                               "Tinlings" => "tinlings",
                               "Crosshall" => "crosshall",
                               "Chapel" => "chapel",
                               "West Ave" => "west ave",
